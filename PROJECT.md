...
| Release Decision | **PASS** — `npm run verify` green at the head of `feat/carebridge-homepage`; deploy `out/` + `public/.htaccess` |
| Open PRs | #8, #9, #10 merged into `main` on 17 Sep 2026. `feat/carebridge-homepage` → `main` is the current PR. |
...
| Last structural change | `/carebridge/` now mirrors Carebridge's own home page (iframe of `public/partners/carebridge/`, generated by `npm run partners:carebridge`); every click leaves for `https://carebridge.education/`; the page is noindex and out of the sitemap like every iframe partner landing; `/partner/carebridge/` redirects to it. The 912-line SMRU-authored `CarebridgeLanding` view is gone. Two shared partner-iframe bugs fixed: the loader overlay stuck when a same-origin frame loaded before hydration, and the frame clipped content that grew after load. |
