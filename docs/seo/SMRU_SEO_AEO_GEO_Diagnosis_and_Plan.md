...
**Scope:** smru.edu.in — brand searches ("St Mary's", "SMRU"), all 6 school searches, and course/programme searches across 90+ programmes; **India and international** (NRI, foreign-national, and the Carebridge UK/Australia/Gulf/Canada/USA career pathways); classic search (Google, Bing), answer engines (Google AI Overviews / AI Mode, Bing Copilot) and generative engines (ChatGPT, Perplexity, Gemini, Claude).
...
### 6.6 International search — the second audience the site does not yet serve

The site already carries a global proposition (the Carebridge pathway: UK, Australia, Gulf, Canada, USA, India; OET/IELTS coaching; licensing-exam prep; "5–8× salary uplift"), but there is **no international-facing information architecture at all**: no international-students section, no NRI/foreign-national admission route, no fees in USD, no visa/FRRO guidance, no `hreflang`, and the only language declaration is `lang="en-IN"` sitewide. Two distinct international audiences are being missed:

**A. Inbound — students outside India who want to study here.** Primary markets for Indian allied-health and nursing programmes: Nepal, Bhutan, Bangladesh, Sri Lanka, Maldives, Afghanistan, Nigeria, Kenya, Tanzania, Ethiopia, Sudan, Yemen, Oman, UAE, Saudi Arabia (NRI children), Malaysia, Mauritius, Fiji. Query families to own: "study BPT in India for international students", "MBBS alternative courses in India for foreign students", "NRI quota physiotherapy India", "B.Sc nursing in India for Nepali students", "rehabilitation courses in India for African students", "cheapest allied health degree in India for international students", "{country} students admission Indian private university".

**B. Outbound — the Carebridge promise.** Students choose an Indian degree *because* it leads abroad. Query families: "physiotherapy degree India recognised in UK / HCPC", "Indian B.Sc nursing NMC UK OSCE", "AHPRA registration Indian physiotherapy degree", "NCLEX eligibility Indian nursing degree", "DHA / HAAD / MOH / Prometric exam after B.Sc nursing India", "occupational therapy India to Canada", "OET coaching with nursing degree India". These are high-intent, low-competition, and nobody in Hyderabad is writing them well.

What to build (all additive — do not disturb the India-facing pages):

1. **`/international/` hub** — who can apply, eligibility equivalence, English-language requirement, fee in INR **and** indicative USD, scholarships for international students, application steps, document list (passport, transcripts, equivalence certificate from AIU, medical, police clearance), student-visa route (Indian student visa, e-FRRO registration within 14 days, bonafide letter), arrival support, hostel and food, a named international-admissions contact and e-mail, and a WhatsApp number in international format. Schema: `WebPage` + `FAQPage` + `ContactPoint` with `areaServed` list and `availableLanguage`.
2. **`/international/{country}/` pages** — only for countries where you can say something real (start with Nepal, Bangladesh, Sri Lanka, Nigeria, Kenya, UAE, Oman, Bhutan). Each: eligibility mapping from that country's school-leaving qualification to Indian 10+2, visa specifics, fee in local currency alongside INR, alumni/student count if any, embassy/attestation steps. Never publish a country page you cannot substantiate — an empty country page is worse than none.
3. **`/international/nri-admissions/`** — NRI/OCI/PIO quota, documentation, fee structure differences.
4. **Global-careers cluster under Carebridge** — one page per destination-profession pair that you can evidence: `/carebridge/uk-nursing/`, `/carebridge/uk-physiotherapy-hcpc/`, `/carebridge/australia-ahpra/`, `/carebridge/gulf-dha-haad/`, `/carebridge/canada/`, `/carebridge/usa-nclex/`. Each explains the licensing pathway honestly (exam, body, timeline, typical cost, what SMRU provides vs what the student must do), with a clear disclaimer that registration is granted by the foreign regulator, not by the university. This is the content that earns international links and AI citations — and it is the one area where SMRU can genuinely out-write every competitor in Telangana.
5. **Programme pages gain an international block:** "International students: eligibility equivalence · fee in USD · is this degree recognised in {UK/Australia/Gulf/Canada}? · visa route" with a link to `/international/`. Add `Course.availableLanguage: en`, and `Course.offers` with a second `Offer` in USD where an international fee exists.
6. **Technical internationalisation:**
   - Keep one URL set (no `/in/` or `/en/` split — you do not have translated content).
   - Change the document language to `lang="en"` with `<link rel="alternate" hreflang="en-IN" href="…">`, `hreflang="en"` and `hreflang="x-default"` all pointing at the same canonical URL. This tells Google the page serves English speakers everywhere, not only India. (Today `lang="en-IN"` plus an India-only signal set suppresses the site in non-India result sets.)
   - In Search Console, leave International Targeting unset (or set to India only if data shows no international value) — a `.edu.in` ccTLD-like domain already skews to India; the hreflang + `x-default` pair is what recovers global reach.
   - Prices: keep INR as the schema `priceCurrency` and show USD as display text, or emit two `Offer` nodes with correct `priceCurrency`.
   - Add `GeoCoordinates` and `areaServed` (IN plus the countries you actively recruit from) to the Organization schema.
   - Serve the same content to every country — no geo-redirects, no cloaking.
7. **International citations** — the off-site list in Section 4 has a global twin: StudyPortals / Bachelorsportal / Mastersportal, EDUopinions, Studyabroad.shiksha, Yocket, Leverage Edu, StudyInIndia (Govt. of India portal — a genuine authority link), AIU membership listing, ICCR where applicable, and country-specific student-agent directories. Each must carry the standard name and the bridge sentence.
8. **Measurement:** add a country dimension to the Search Console reporting in Section 9 and track the two international query families separately; they will move on a different, slower curve than the Hyderabad terms.

Sequencing: this is **P2** work — it should start only after P0 (naming, redirects, sitemap) and the Google Business Profile are done, because an international audience that searches "St Mary's University" hits exactly the same entity collision, plus three more namesakes (Texas, Twickenham, Halifax). The naming fix is a prerequisite for the international push, not a parallel track.
